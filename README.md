...
|---|---|---|
| 1 | **Ajouter un article** | Saisir nom, référence, prix et quantité |
| 2 | **Afficher tous les articles** | Lister l'ensemble du stock |
| 3 | **Chercher par référence** | Retrouver un article via son numéro de référence |
| 4 | **Chercher par nom** | Retrouver un article via son nom |
| 5 | **Chercher par intervalle de prix** | Filtrer les articles entre un prix minimum et maximum |
| 6 | **Supprimer par référence** | Retirer un article du stock |
| 7 | **Modifier par référence** | Mettre à jour le nom, le prix ou la quantité d'un article |
| 8 | **Sauvegarder** | Exporter le stock dans un fichier `Stock.txt` |
...
```bash
g++ -std=c++11 -o graja_store main.cpp
./graja_store
```
...
```bash
g++ -std=c++11 -o graja_store.exe main.cpp
graja_store.exe
```
